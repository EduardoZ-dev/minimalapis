...
namespace MinimalAPIPeliculas.Endpoints
{
    public  static class ActoresEndpoints
    {
        ...
        public static RouteGroupBuilder MapActores(this RouteGroupBuilder group)
        {
            ...
            group.MapGet("/{id}", ObtenerPorId);
            group.MapGet("/obtenerPorNombre/{nombre}", ObtenerPorNombre);
            group.MapPost("/", Crear).DisableAntiforgery()
                .AddEndpointFilter<FiltroValidaciones<CrearActorDTO>>()
                .RequireAuthorization("esadmin")
                .WithOpenApi();
            group.MapPut("/{id}", Actualizar).DisableAntiforgery().DisableAntiforgery()
                .AddEndpointFilter<FiltroValidaciones<CrearActorDTO>>()
                .RequireAuthorization("esadmin")
                .WithOpenApi();
            group.MapDelete("/{id}", Borrar).RequireAuthorization("esadmin");
            return group;
        }
        ...
        static async Task<Ok<List<ActorDTO>>> ObtenerPorNombre(string nombre, IRepositorioActores repositorio,
            IMapper mapper)
        {
        ...
        static async Task<Results<NoContent, NotFound>> Actualizar(int id, [FromForm] 
        CrearActorDTO crearActorDTO, IRepositorioActores repositorio,
