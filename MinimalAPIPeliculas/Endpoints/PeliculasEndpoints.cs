...
namespace MinimalAPIPeliculas.Endpoints
{
    public static class PeliculasEndpoints
    {
        ...
        public static RouteGroupBuilder MapPeliculas(this RouteGroupBuilder group)
        {
            ...
            group.MapPut("/{id}", Actualizar).DisableAntiforgery()
                .AddEndpointFilter<FiltroValidaciones<CrearPeliculaDTO>>()
                .RequireAuthorization("esadmin")
                .WithOpenApi();
            group.MapDelete("/{id}", Borrar).RequireAuthorization("esadmin");
            group.MapPost("/{id}/asignargeneros", AsignarGeneros).RequireAuthorization("esadmin");
            group.MapPost("/{id}/asignaractores", AsignarActores).RequireAuthorization("esadmin");
            ...
            IRepositorioPeliculas repositorioPeliculas, IRepositorioGeneros repositorioGeneros)
        {
            ...
            if(generosExistentes.Count != generosIds.Count)
            {
                ...
                return TypedResults.BadRequest($"Los geéneros de id {string.Join(",", generosNoExistentes)} no existen");
